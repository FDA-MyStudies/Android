...
public class SurveyActivitiesFragment extends Fragment implements ApiCall.OnAsyncRequestComplete,
  ...
  private void initializeXMLId(View view) {
    ...
    mSurveyActivitiesRecyclerView = (RecyclerView) view
        .findViewById(R.id.mSurveyActivitiesRecyclerView);
  ...
  private void bindEvents() {
    ...
    mFilterBtn.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View view) {
        ...
        CustomActivitiesDailyDialogClass c = new CustomActivitiesDailyDialogClass(mContext,
            mScheduledTime, filterPos, true, SurveyActivitiesFragment.this);
        c.show();
      }
    });
  ...
  private void getStudyUpdateFomWS(boolean isSwipeToRefresh) {
    ...
    StudyList studyList = dbServiceSubscriber
        .getStudiesDetails(((SurveyActivity) mContext).getStudyId(), mRealm);
    String url = URLs.STUDY_UPDATES + "?studyId=" + ((SurveyActivity) mContext).getStudyId()
        + "&studyVersion=" + studyList.getStudyVersion();
    WCPConfigEvent wcpConfigEvent = new WCPConfigEvent("get", url, STUDY_UPDATES, mContext,
        StudyUpdate.class, null, header, null, false, this);
  ...
  private void setRecyclerView() {
    ...
    WCPConfigEvent wcpConfigEvent = new WCPConfigEvent("get", url, ACTIVTTYLIST_RESPONSECODE,
        mContext, ActivityListData.class, null, header, null, false, this);
  ...
  private class callConsentMetaData extends AsyncTask<String, Void, String> {
    ...
    protected String doInBackground(String... params) {
      ...
      String url = URLs.BASE_URL_WCP_SERVER + URLs.CONSENT_METADATA + "?studyId="
      ...
      if (connectionDetector.isConnectingToInternet()) {
        ...
        if (responseCode.equalsIgnoreCase("0") && response.equalsIgnoreCase("timeout")) {
          ...
          response = "error";
        } else if (Integer.parseInt(responseCode) >= 201 && Integer.parseInt(responseCode) < 300
        ...
        } else if (Integer.parseInt(responseCode) == HttpURLConnection.HTTP_OK && !response
            .equalsIgnoreCase("")) {
          response = response;
        } else {
    ...
    protected void onPostExecute(String result) {
      ...
      if (response != null) {
        if (response.equalsIgnoreCase("session expired")) {
          ...
          AppController.getHelperProgressDialog().dismissDialog();
          Toast.makeText(mContext, mContext.getResources().getString(R.string.connection_timeout),
  ...
  private void startConsent(Consent consent, String type) {
    ...
    List<Step> consentStep = consentBuilder
        .createsurveyquestion(mContext, consent, studyList.getTitle());
    Task consentTask = new OrderedTask(StudyFragment.CONSENT, consentStep);
    Intent intent = CustomConsentViewTaskActivity
        .newIntent(mContext, consentTask, ((SurveyActivity) mContext).getStudyId(), "", mTitl,
            eligibilityType, "update");
  ...
  private void genarateConsentPDF(String signatureBase64) {
    try {
      ...
      if (eligibilityConsent != null && eligibilityConsent.getConsent() != null
          && eligibilityConsent.getConsent().getReview() != null
          && eligibilityConsent.getConsent().getReview().getSignatureContent() != null
          && !eligibilityConsent.getConsent().getReview().getSignatureContent()
          .equalsIgnoreCase("")) {
        consentItem = new Paragraph(Html.fromHtml(
            eligibilityConsent.getConsent().getReview().getSignatureContent().toString())
            .toString());
      } else if (eligibilityConsent != null && eligibilityConsent.getConsent() != null
        ...
        if (eligibilityConsent.getConsent().getVisualScreens().size() > 0) {
          ...
          for (int i = 0; i < eligibilityConsent.getConsent().getVisualScreens().size(); i++) {
            docBuilder.append(
                "<div>  <h4>" + eligibilityConsent.getConsent().getVisualScreens().get(i).getTitle()
                    + "<h4> </div>");
            docBuilder.append("</br>");
            docBuilder.append(
                "<div>" + eligibilityConsent.getConsent().getVisualScreens().get(i).getHtml()
                    + "</div>");
            docBuilder.append("</br>");
            docBuilder.append("</br>");
          }
      ...
      table1.addCell(getCell(mContext.getResources().getString(R.string.participans_name),
          PdfPCell.ALIGN_CENTER));
      table1.addCell(getCell(mContext.getResources().getString(R.string.participants_signature),
          PdfPCell.ALIGN_CENTER));
      table1.addCell(
          getCell(mContext.getResources().getString(R.string.date), PdfPCell.ALIGN_CENTER));
    ...
    if (responseCode == STUDY_UPDATES) {
      ...
      if (studyUpdate.getStudyUpdateData().getStatus()
        ...
        if (studyUpdate.getStudyUpdateData().isResources()) {
          dbServiceSubscriber
              .deleteResourcesFromDb(mContext, ((SurveyActivity) mContext).getStudyId());
        }
        if (studyUpdate.getStudyUpdateData().isInfo()) {
          dbServiceSubscriber
              .deleteStudyInfoFromDb(mContext, ((SurveyActivity) mContext).getStudyId());
        }
        if (studyUpdate.getStudyUpdateData().isConsent()) {
          callConsentMetaDataWebservice();
        } else {
          StudyList studyList = dbServiceSubscriber
              .getStudyTitle(((SurveyActivity) mContext).getStudyId(), mRealm);
          dbServiceSubscriber
              .updateStudyPreferenceVersionDB(mContext, ((SurveyActivity) mContext).getStudyId(),
      ...
      HashMap<String, String> header = new HashMap();
      header.put("auth", AppController.getHelperSharedPreference()
          .readPreference(mContext, mContext.getResources().getString(R.string.auth), ""));
      header.put("userId", AppController.getHelperSharedPreference()
      ...
      RegistrationServerConfigEvent registrationServerConfigEvent = new RegistrationServerConfigEvent(
          "get", url, GET_PREFERENCES, mContext, ActivityData.class, null, header, null, false,
          this);
      ...
      activityDataDB = dbServiceSubscriber
          .getActivityPreference(((SurveyActivity) mContext).getStudyId(), mRealm);
      ...
      if (loginData != null) {
        ...
        dbServiceSubscriber.updateActivityPreferenceDB(mContext, mActivityId,
            ((SurveyActivity) mContext).getStudyId(), mCurrentRunId,
            SurveyActivitiesFragment.IN_PROGRESS, mActivityStatusData.getTotalRun(),
            mActivityStatusData.getCompletedRun(), mActivityStatusData.getMissedRun(),
            mActivityVersion);
      } else {
        Toast.makeText(mContext, R.string.unable_to_parse, Toast.LENGTH_SHORT).show();
      }
  ...
  private void calculateStartAnsEndDateForActivities() {
    ...
    if (activityListData == null) {
      ActivityListData activityListDataTemp = dbServiceSubscriber
          .getActivities(((SurveyActivity) mContext).getStudyId(), mRealm);
    ...
    if (activityDataDB == null) {
      ActivityData activityDataTemp = dbServiceSubscriber
          .getActivityPreference(((SurveyActivity) mContext).getStudyId(), mRealm);
    ...
    if (activityListData != null && activityListData.getActivities() != null
      ...
      for (int i = 0; i < activityListData.getActivities().size(); i++) {
        if (activityListData.getActivities().get(i).getSchedulingType() != null) {
          if (activityListData.getActivities().get(i).getSchedulingType()
              .equalsIgnoreCase("AnchorDate")) {
            Studies studies = dbServiceSubscriber
                .getStudies(((SurveyActivity) mContext).getStudyId(), mRealm);
            if (activityListData.getActivities().get(i).getAnchorDate().getSourceType()
              ...
              anchorDateSchedulingDetails
                  .setSchedulingType(activityListData.getActivities().get(i).getSchedulingType());
              ...
              anchorDateSchedulingDetails
                  .setTargetActivityId(activityListData.getActivities().get(i).getActivityId());

              for (int j = 0; j < activityDataDB.getActivities().size(); j++) {
                if (activityDataDB.getActivities().get(j).getActivityId()
                    .equalsIgnoreCase(anchorDateSchedulingDetails.getSourceActivityId())) {
                  anchorDateSchedulingDetails
                      .setActivityState(activityDataDB.getActivities().get(j).getStatus());
                  mArrayList.add(anchorDateSchedulingDetails);
                  break;
                }
              }
            } else if (activityListData.getActivities().get(i).getAnchorDate().getSourceType()
...
//                                }
              ...
              for (int j = 0; j < activityDataDB.getActivities().size(); j++) {
                if (activityDataDB.getActivities().get(j).getActivityId()
                  ...
                  anchorDateSchedulingDetails
                      .setEndDate(activityDataDB.getActivities().get(j).getActivityEndDate());
                  anchorDateSchedulingDetails.setLastUpdatedDate(
                      activityDataDB.getActivities().get(j).getLastModifiedDate());
                  anchorDateSchedulingDetails
                      .setActivities(activityDataDB.getActivities().get(j));
//                                    if (!activityDataDB.getActivities().get(i).getAnchorDateVersion().equalsIgnoreCase(anchorDateSchedulingDetails.getVersion())) {
                  ...
                  try {
                    if (activityDataDB.getActivities().get(j).getActivityStartDate() != null
                        ...
                        && !activityDataDB.getActivities().get(j).getActivityEndDate()
                        .equalsIgnoreCase("")) {
                      startDate.setTime(new SimpleDateFormat("dd/MM/yyyy")
                          .parse(activityDataDB.getActivities().get(j)
                              .getActivityStartDate()));
                      endDate.setTime(new SimpleDateFormat("dd/MM/yyyy")
                          .parse(activityDataDB.getActivities().get(j)
                              .getActivityEndDate()));
        ...
        if (activityListData.getActivities().get(i).getSchedulingType() != null) {
          if (activityListData.getActivities().get(i).getSchedulingType()
            ...
            for (int j = 0; j < mArrayList.size(); j++) {
              if (activityListData.getActivities().get(i).getActivityId()
...
//            boolean updateRun = true;
    ...
    StudyUpdate studyUpdate = dbServiceSubscriber
        .getStudyUpdateById(((SurveyActivity) mContext).getStudyId(), mRealm);
    ...
    if (activityDataDB != null && activityListData != null) { //&& activityData != null
      for (int j = 0; j < activityListData.getActivities().size(); j++) {
        ...
        for (int i = 0; i < activityDataDB.getActivities().size(); i++) {
          ...
          if (activityDataDB.getActivities().get(i).getActivityId()
              .equalsIgnoreCase(activityListData.getActivities().get(j).getActivityId())) {
            activityAvailable = true;
            if (!activityDataDB.getActivities().get(i).getActivityVersion()
              ...
              dbServiceSubscriber.UpdateActivitiesWSVersion(
                  ...
                  activityListData.getStudyId(), mRealm,
                  activityListData.getActivities().get(j).getActivityVersion());
              dbServiceSubscriber.updateActivityPreferenceVersion(mContext,
                  activityListData.getActivities().get(j).getActivityVersion(),
              ...
              if (activityIds.contains(activityDataDB.getActivities().get(i).getActivityId())) {
                ...
                runIds
                    .set(activityIds.indexOf(activityDataDB.getActivities().get(i).getActivityId()),
        ...
        if (!activityAvailable) {
          ActivitiesWS activitiesWS = dbServiceSubscriber
              .getActivityObj(activityListData.getActivities().get(j).getActivityId(),
                  activityListData.getStudyId(), mRealm);
          if (activitiesWS != null && !activitiesWS.getActivityVersion()
              .equalsIgnoreCase(activityListData.getActivities().get(j).getActivityVersion())) {
            mActivityUpdated = true;
            //update ActivityWS DB with new version
            dbServiceSubscriber
                .UpdateActivitiesWSVersion(activityListData.getActivities().get(j).getActivityId(),
                    activityListData.getStudyId(), mRealm,
                    activityListData.getActivities().get(j).getActivityVersion());
  ...
  private void setDates(int i, int j, ArrayList<String> activityIds, ArrayList<String> runIds) {
    ...
    if (mArrayList.get(j).getAnchorDate() != null && !mArrayList.get(j).getAnchorDate()
      ...
      if (activityListData.getActivities().get(i).getAnchorDate() != null
        ...
        if (!activityListData.getActivities().get(i).getAnchorDate().getStart().getTime()
            .equalsIgnoreCase("")) {
      ...
      if (activityListData.getActivities().get(i).getAnchorDate() != null
        ...
        if (!activityListData.getActivities().get(i).getAnchorDate().getEnd().getTime()
            .equalsIgnoreCase("")) {
      ...
      RealmResults<ActivityRun> runs = dbServiceSubscriber
          .getAllActivityRunFromDB(activityListData.getStudyId(),
              activityListData.getActivities().get(i).getActivityId(), mRealm);
      ...
      if (activityListData.getActivities().get(i).getFrequency().getType()
          .equalsIgnoreCase("One Time")) {
        Calendar calendar;
        if (calculateStartDate && activityListData.getActivities().get(i).getAnchorDate() != null
            && activityListData.getActivities().get(i).getAnchorDate().getStart() != null) {
          ...
          try {
            ...
            calendar.add(Calendar.DATE,
                activityListData.getActivities().get(i).getAnchorDate().getStart().getAnchorDays());
          } catch (ParseException e) {
            e.printStackTrace();
          }
...
//                                            activityListData.getActivities().get(i).setStartTime(dateSimpleDateFormat.format(calendar.getTime()) + "T" + startTime + ".000" + timezoneSimpleDateFormat.format(calendar.getTime()));
        ...
        if (calculateEndDate && activityListData.getActivities().get(i).getAnchorDate() != null
            && activityListData.getActivities().get(i).getAnchorDate().getEnd() != null) {
          calendar = Calendar.getInstance();
          try {
            date = simpleDateFormat.parse(mArrayList.get(j).getAnchorDate());
            calendar.setTime(date);
            calendar.add(Calendar.DATE,
                activityListData.getActivities().get(i).getAnchorDate().getEnd().getAnchorDays());
          } catch (ParseException e) {
            e.printStackTrace();
          }
...
//                                            activityListData.getActivities().get(i).setEndTime(dateSimpleDateFormat.format(calendar.getTime()) + "T" + endTime + ".000" + timezoneSimpleDateFormat.format(calendar.getTime()));
      ...
      } else if (activityListData.getActivities().get(i).getFrequency().getType()
          .equalsIgnoreCase("Daily")) {
        if (calculateStartDate && activityListData.getActivities().get(i).getStartTime()
            .equalsIgnoreCase("")) {
          Calendar calendar = Calendar.getInstance();
          try {
            date = simpleDateFormat.parse(mArrayList.get(j).getAnchorDate());
...
//                                            activityListData.getActivities().get(i).setStartTime(dateSimpleDateFormat.format(calendar.getTime()) + "T" + startTime + ".000" + timezoneSimpleDateFormat.format(calendar.getTime()));
        ...
        if (calculateEndDate && activityListData.getActivities().get(i).getEndTime()
            .equalsIgnoreCase("")) {
          Calendar calendar = Calendar.getInstance();
          try {
            calendar.setTime(
          ...
          calendar.add(Calendar.DATE,
              activityListData.getActivities().get(i).getAnchorDate().getEnd().getRepeatInterval());
//                                            activityListData.getActivities().get(i).setEndTime(dateSimpleDateFormat.format(calendar.getTime()) + "T" + endTime + ".000" + timezoneSimpleDateFormat.format(calendar.getTime()));
          setEndDate(calendar.getTime(), endTime, i, j);
        }
      } else if (activityListData.getActivities().get(i).getFrequency().getType()
...
//                                            activityListData.getActivities().get(i).setEndTime(dateSimpleDateFormat.format(calendar.getTime()) + "T" + endTime + ".000" + timezoneSimpleDateFormat.format(calendar.getTime()));
        ...
        if (activityListData.getActivities().get(i).getStartTime().equalsIgnoreCase("")
            && activityListData.getActivities().get(i).getEndTime().equalsIgnoreCase("")) {
          Calendar startCalendar;
          Calendar endCalendar;
          for (int k = 0;
              k < activityListData.getActivities().get(i).getFrequency().getAnchorRuns().size();
              k++) {
            ...
            if (activityListData.getActivities().get(i).getSchedulingType()
              ...
              try {
                ...
                startCalendar.add(Calendar.DATE,
                    activityListData.getActivities().get(i).getFrequency().getAnchorRuns().get(k)
                        .getStartDays());
              } catch (ParseException e) {
                e.printStackTrace();
              }
              activityListData.getActivities().get(i).getFrequency().getRuns().get(k).setStartTime(
                  dateSimpleDateFormat.format(startCalendar.getTime()) + "T" + startTime + ".000"
              ...
              activityListData.getActivities().get(i).getFrequency().getRuns().get(k).setEndTime(
                  dateSimpleDateFormat.format(endCalendar.getTime()) + "T" + endTime + ".000"
                      + timezoneSimpleDateFormat.format(endCalendar.getTime()));

              activityListData.getActivities().get(i).setStartTime(
                  activityListData.getActivities().get(i).getFrequency().getRuns().get(0)
                      .getStartTime());
              activityListData.getActivities().get(i).setEndTime(
                  activityListData.getActivities().get(i).getFrequency().getRuns().get(k)
                      .getEndTime());
            }
  ...
  private boolean isDatesOverlapping(Activities activities, int i,
      Calendar startCalendar, Calendar endCalendar, SimpleDateFormat simpleDateFormat) {
    boolean isOverlapping = false;

    try {
      for (int l = 0; l < activities.getCustomScheduleRuns().size(); i++) {
        if (((startCalendar.getTime().after(
            simpleDateFormat.parse(activities.getCustomScheduleRuns().get(l).getActivityStartDate()))
            || startCalendar.getTime().equals(
            simpleDateFormat.parse(activities.getCustomScheduleRuns().get(l).getActivityStartDate())))
            && (startCalendar.getTime().before(
            simpleDateFormat.parse(activities.getCustomScheduleRuns().get(l).getActivityEndDate()))
            || startCalendar.getTime().equals(
            simpleDateFormat.parse(activities.getCustomScheduleRuns().get(l).getActivityEndDate()))))

            || ((endCalendar.getTime().after(
            simpleDateFormat.parse(activities.getCustomScheduleRuns().get(l).getActivityStartDate()))
            || endCalendar.getTime().equals(
            simpleDateFormat.parse(activities.getCustomScheduleRuns().get(l).getActivityStartDate())))
            && (endCalendar.getTime().before(
            simpleDateFormat.parse(activities.getCustomScheduleRuns().get(l).getActivityEndDate()))
            || endCalendar.getTime().equals(simpleDateFormat
            .parse(activities.getCustomScheduleRuns().get(l).getActivityEndDate()))))) {
          isOverlapping = true;
          break;
        }
      }
  ...
  private void setStartDateEndDateAndParticipantProperty(int i, int j, int k,
    ...
    try {
      ...
      startCalendar.add(Calendar.DATE,
          activityListData.getActivities().get(i).getFrequency().getAnchorRuns().get(k)
              .getStartDays());
      String startDateFull = dateSimpleDateFormat.format(startCalendar.getTime()) + "T" + startTime + ".000"
          + timezoneSimpleDateFormat.format(startCalendar.getTime());
      ...
      endCalendar.add(Calendar.DATE,
          activityListData.getActivities().get(i).getFrequency().getAnchorRuns().get(k)
              .getEndDays());
      String endDatefull = dateSimpleDateFormat.format(endCalendar.getTime()) + "T" + endTime + ".000"
          + timezoneSimpleDateFormat.format(endCalendar.getTime());
      endCalendar.setTime(simpleDateFormat.parse(endDatefull));

      if (activityListData.getActivities().get(i).getFrequency().isAddNewRuns()) {
        if (mArrayList.get(j).getCurrentStatus().equalsIgnoreCase("Upcoming")) {
          if (!isDatesOverlapping(mArrayList.get(j).getActivities(), i, startCalendar, endCalendar, simpleDateFormat)) {
            ...
            updateRunsToActivityListAPIData(startDateFull, endDatefull, i, j, k,simpleDateFormat, endCalendar);
          }
        } else {
          if (!isDatesOverlapping(mArrayList.get(j).getActivities(), i, startCalendar, endCalendar, simpleDateFormat) && endCalendar.getTime().after(simpleDateFormat.parse(mArrayList.get(j).getLastUpdatedDate()))) {
            updateRunsToActivityListAPIData(startDateFull, endDatefull, i, j, k,simpleDateFormat, endCalendar);
          }
        }
      } else {
        addNewRunsFalseScenario(i, j, k, simpleDateFormat, startTime, endTime);
      }
    } catch (ParseException e) {
      e.printStackTrace();
    }
  ...
  private void addNewRunsFalseScenario(int i, int j, int k, SimpleDateFormat simpleDateFormat, String startTime, String endTime) {
    Calendar startCalendar = Calendar.getInstance();
    Calendar endCalendar = Calendar.getInstance();
    //start runs
    try {
      ...
      if (activityListData.getActivities().get(i).getFrequency().getRuns().get(k).getStartTime().equalsIgnoreCase("")
          || simpleDateFormat.parse(activityListData.getActivities().get(i).getFrequency().getRuns().get(k).getStartTime()).after(simpleDateFormat.parse(mArrayList.get(j).getLastUpdatedDate()))) {
        activityListData.getActivities().get(i).getFrequency().getRuns().get(k).setStartTime(
            dateSimpleDateFormat.format(startCalendar.getTime()) + "T" + startTime + ".000"
                + timezoneSimpleDateFormat.format(startCalendar.getTime()));
      }
    ...
    //end runs
      ...
      if (activityListData.getActivities().get(i).getFrequency().getRuns().get(k).getEndTime().equalsIgnoreCase("")
          || simpleDateFormat.parse(activityListData.getActivities().get(i).getFrequency().getRuns().get(k).getEndTime()).after(simpleDateFormat.parse(mArrayList.get(j).getLastUpdatedDate()))) {
        activityListData.getActivities().get(i).getFrequency().getRuns().get(k).setEndTime(
            dateSimpleDateFormat.format(endCalendar.getTime()) + "T" + endTime + ".000"
                + timezoneSimpleDateFormat.format(endCalendar.getTime()));
      }
    ...
    activityListData.getActivities().get(i).setEndTime(
        activityListData.getActivities().get(i).getFrequency().getRuns().get(k)
            .getEndTime());
    dbServiceSubscriber.deleteActivityRunsFromDb(mContext,
        activityListData.getActivities().get(i).getActivityId(), activityListData.getStudyId());
  }

  private void updateRunsToActivityListAPIData(String startDate, String endDate, int i, int j,
      int k, SimpleDateFormat simpleDateFormat, Calendar endCalendar) {
    try {
      activityListData.getActivities().get(i).getFrequency().getRuns().clear();

      for(int m = 0; m < mArrayList.get(j).getActivities().getCustomScheduleRuns().size(); m++){
        if(simpleDateFormat.parse(mArrayList.get(j).getActivities().getCustomScheduleRuns().get(m).getActivityStartDate()).after(endCalendar.getTime())) {
        ...
        FrequencyRuns frequencyRuns = new FrequencyRuns();
        frequencyRuns.setStartTime(mArrayList.get(j).getActivities().getCustomScheduleRuns().get(m).getActivityStartDate());
        frequencyRuns.setEndTime(mArrayList.get(j).getActivities().getCustomScheduleRuns().get(m).getActivityEndDate());
        activityListData.getActivities().get(i).getFrequency().getRuns().add(frequencyRuns);
        activityListData.getActivities().get(i).setStartTime(
            activityListData.getActivities().get(i).getFrequency().getRuns().get(0)
                .getStartTime());
  ...
  private void setStartDate(Date time, String startTime, int pos, int userRegListPos) {
    ...
    dbServiceSubscriber.deleteActivityRunsFromDb(mContext,
        activityListData.getActivities().get(pos).getActivityId(), activityListData.getStudyId());
    if (setDate) {
      activityListData.getActivities().get(pos).setLastModifiedDate(new Date().toString());
      activityListData.getActivities().get(pos).setStartTime(
          dateSimpleDateFormat.format(time) + "T" + startTime + ".000"
              + timezoneSimpleDateFormat
              .format(time));
    } else {
  ...
  private void setEndDate(Date time, String endTime, int pos, int userRegListPos) {
    ...
    if (setDate) {
      activityListData.getActivities().get(pos).setLastModifiedDate(new Date().toString());
      activityListData.getActivities().get(pos).setEndTime(
          dateSimpleDateFormat.format(time) + "T" + endTime + ".000" + timezoneSimpleDateFormat
              .format(time));
    } else {
      activityListData.getActivities().get(pos)
          .setStartTime(mArrayList.get(userRegListPos).getStartDate());
  ...
  private void callGetStudyInfoWebservice() {
    ...
    WCPConfigEvent wcpConfigEvent = new WCPConfigEvent("get", url, STUDY_INFO, getActivity(),
        StudyHome.class, null, header, null, false, this);
  ...
  private void checkForNotification() {
    if (((SurveyActivity) mContext).mFrom.equalsIgnoreCase("NotificationActivity")
      ...
      for (int i = 0; i < studyVideoAdapter.items.size(); i++) {
        if (studyVideoAdapter.items.get(i).getActivityId() != null && studyVideoAdapter.items.get(i)
            .getActivityId().equalsIgnoreCase(((SurveyActivity) mContext).mActivityId)) {
          position = i;
          break;
        }
      }
      ...
      if (paused) {
        ...
        if (studyVideoAdapter.mStatus.get(position)
          ...
          if (studyVideoAdapter.mCurrentRunStatusForActivities.get(position).isRunIdAvailable()) {
            getActivityInfo(studyVideoAdapter.items.get(position).getActivityId(),
            ...
            Toast.makeText(mContext, mContext.getResources().getString(R.string.survey_message),
                Toast.LENGTH_SHORT).show();
          }
        } else if (studyVideoAdapter.mStatus.get(position)
        ...
        } else if (studyVideoAdapter.mCurrentRunStatusForActivities.get(position).getStatus()
  ...
  private void displayData(ActivityListData activityListData, ArrayList<String> activityIds,
      ArrayList<String> runIds, String errormsg) {
  ...
  public void clicked(int positon) {
    ...
    studyVideoAdapter = new SurveyActivitiesListAdapter(mContext, filter.getActivitiesArrayList1(),
        filter.getStatus(), filter.getCurrentRunStatusForActivities(),
        SurveyActivitiesFragment.this, paused);
    mSurveyActivitiesRecyclerView.setLayoutManager(new LinearLayoutManager(mContext));
    mSurveyActivitiesRecyclerView.setAdapter(studyVideoAdapter);
  }

  private class CalculateRuns extends
      AsyncTask<ArrayList<ActivitiesWS>, Void, ArrayList<ActivitiesWS>> {
    ...
    CalculateRuns(ActivityListData activityListData, ArrayList<String> activityIds,
        ArrayList<String> runIds, String errormsg) {
    ...
    protected ArrayList<ActivitiesWS> doInBackground(ArrayList<ActivitiesWS>... params) {
      ...
      ActivityListData activityListDataDB = dbServiceSubscriber
          .getActivities(((SurveyActivity) mContext).getStudyId(), mRealm);
      if (activityListDataDB != null && activityListDataDB.getActivities() != null
        ...
        if (activityListData == null) {
          ...
          for (int i = 0; i < activityListDataDB.getActivities().size(); i++) {
            boolean activityAvailable = false;
            for (int j = 0; j < activityListData.getActivities().size(); j++) {
              if (activityListData.getActivities().get(j).getActivityId()
            ...
            for (int i = 0; i < activityListDataDB.getActivities().size(); i++) {
              if (activityListData.getActivities().get(j).getActivityId()
                ...
                if (activityListData.getActivities().get(j).getState().equalsIgnoreCase(DELETE)
                    && activityListDataDB.getActivities().get(i).getState()
                    .equalsIgnoreCase(ACTIVE)) {
                  RealmResults<ActivityRun> activityRuns = dbServiceSubscriber
                      .getAllActivityRunFromDB(((SurveyActivity) mContext).getStudyId(),
                          activityListData.getActivities().get(j).getActivityId(), mRealm);
                  try {
                  ...
                  dbServiceSubscriber
                      .saveActivityState(activityListDataDB.getActivities().get(i), mRealm);
                }
              }
            }
            if (!activityAvailable) {
      ...
      if (activityListData2 != null) {
        ...
        ActivityData activityData = dbServiceSubscriber
            .getActivityPreference(((SurveyActivity) mContext).getStudyId(), mRealm);

        Date joiningDate = survayScheduler
            .getJoiningDateOfStudy(studyPreferences, ((SurveyActivity) mContext).getStudyId());

        Date currentDate = new Date();

        if (mActivityUpdated) {
          dbServiceSubscriber
              .deleteMotivationalNotification(mContext, ((SurveyActivity) mContext).getStudyId());
        }
        ...
        for (int i = 0; i < activitiesArrayList.size(); i++) {
          ...
          if (activitiesArrayList.get(i) != null
              && activitiesArrayList.get(i).getSchedulingType() != null && activitiesArrayList
              .get(i).getSchedulingType().equalsIgnoreCase("AnchorDate")) {
              ...
              if ((activitiesArrayList.get(i).getEndTime().equalsIgnoreCase("")
                ...
                try {
                  starttime = simpleDateFormat
                      .parse(activitiesArrayList.get(i).getStartTime().split("\\.")[0]);
                } catch (ParseException e) {
                  e.printStackTrace();
                }
                try {
                  endtime = simpleDateFormat
                      .parse(activitiesArrayList.get(i).getEndTime().split("\\.")[0]);
                } catch (ParseException e) {
                  e.printStackTrace();
                } catch (Exception e1) {
                  e1.printStackTrace();
          ...
          RealmResults<ActivityRun> activityRuns = dbServiceSubscriber
              .getAllActivityRunFromDB(((SurveyActivity) mContext).getStudyId(),
                  activitiesArrayList.get(i).getActivityId(), mRealm);

          boolean deleted = false;
          for (int j = 0; j < activitiesWSesDeleted.size(); j++) {
            if (activitiesWSesDeleted.get(j).getActivityId()
          ...
          if (updateRun || activityRuns == null || activityRuns.size() == 0) {
            if (!deleted) {
              survayScheduler
                  .setRuns(activitiesArrayList.get(i), ((SurveyActivity) mContext).getStudyId(),
                      starttime, endtime, joiningDate, mContext);
            }
            ...
            if (activityIds.contains(activitiesArrayList.get(i).getActivityId())) {
              dbServiceSubscriber
                  .deleteActivityRunsFromDb(mContext, activitiesArrayList.get(i).getActivityId(),
                      ((SurveyActivity) mContext).getStudyId());
              if (!deleted) {
                survayScheduler
                    .setRuns(activitiesArrayList.get(i),
                        ((SurveyActivity) mContext).getStudyId(),
                        starttime, endtime, joiningDate, mContext);
              }
              // delete activity object that used for survey
              dbServiceSubscriber
                  .deleteActivityObjectFromDb(mContext, activitiesArrayList.get(i).getActivityId(),
                      ((SurveyActivity) mContext).getStudyId());
              for (int j = 0; j < activityData.getActivities().size(); j++) {
                if (activitiesArrayList.get(i).getActivityId()
                    .equalsIgnoreCase(activityData.getActivities().get(j).getActivityId())) {
                  if (!activityData.getActivities().get(j).getStatus()
                      .equalsIgnoreCase(YET_To_START)) {
                    // Delete response data
                    if (!runIds.get(activityIds.indexOf(activitiesArrayList.get(i).getActivityId()))
                        .equalsIgnoreCase("-1")) {
                      dbServiceSubscriber.deleteResponseDataFromDb(mContext,
                          ((SurveyActivity) mContext).getStudyId() + "_STUDYID_"
                              + activitiesArrayList.get(i).getActivityId() + "_" + runIds.get(
          ...
          if (!deleted) {
            ActivityStatus activityStatus = survayScheduler
                .getActivityStatus(activityData, ((SurveyActivity) mContext).getStudyId(),
                    activitiesArrayList.get(i).getActivityId(), calendarCurrentTime.getTime());
            ...
            if (!activitiesArrayList.get(i).getState().equalsIgnoreCase("deleted")) {
              ...
              NotificationModuleSubscriber notificationModuleSubscriber = new NotificationModuleSubscriber(
                  dbServiceSubscriber, mRealm);
              try {
                notificationModuleSubscriber.cancleActivityLocalNotificationByIds(mContext,
              ...
              try {
                notificationModuleSubscriber.cancleResourcesLocalNotificationByIds(mContext,
                    activitiesArrayList.get(i).getActivityId(),
                    ((SurveyActivity) mContext).getStudyId());
              } catch (Exception e) {
                e.printStackTrace();
            ...
            dbServiceSubscriber.deleteActivityWSList(mContext, activityListDataDB,
                activitiesArrayList.get(i).getActivityId());
          }
      ...
      for (int i = 0; i < currentactivityList.size(); i++) {
        for (int j = i; j < currentactivityList.size(); j++) {
          try {
            if (AppController.getDateFormat().parse(currentactivityList.get(i).getStartTime())
                .after(AppController.getDateFormat()
      ...
      for (int i = 0; i < currentactivityList.size(); i++) {
        if (currentActivityStatus.get(i).getStatus()
            .equalsIgnoreCase(SurveyActivitiesFragment.YET_To_START) || currentActivityStatus.get(i)
            .getStatus().equalsIgnoreCase(SurveyActivitiesFragment.IN_PROGRESS)) {
      ...
      for (int i = 0; i < upcomingactivityList.size(); i++) {
        for (int j = i; j < upcomingactivityList.size(); j++) {
          try {
            if (AppController.getDateFormat().parse(upcomingactivityList.get(i).getStartTime())
                .after(AppController.getDateFormat()
      ...
      for (int i = 0; i < completedactivityList.size(); i++) {
        for (int j = i; j < completedactivityList.size(); j++) {
          try {
            if (AppController.getDateFormat().parse(completedactivityList.get(i).getStartTime())
                .after(AppController.getDateFormat()
      ...
      Filter filter = getFilterList();
      studyVideoAdapter = new SurveyActivitiesListAdapter(mContext,
          filter.getActivitiesArrayList1(), filter.getStatus(),
          filter.getCurrentRunStatusForActivities(), SurveyActivitiesFragment.this, paused);
    ...
    protected void onPostExecute(ArrayList<ActivitiesWS> result) {
      ...
      AppController.getHelperSharedPreference()
          .writePreference(mContext, mContext.getResources().getString(R.string.completedRuns),
              "" + completed);
      AppController.getHelperSharedPreference()
          .writePreference(mContext, mContext.getResources().getString(R.string.missedRuns),
              "" + missed);
      AppController.getHelperSharedPreference()
          .writePreference(mContext, mContext.getResources().getString(R.string.totalRuns),
              "" + total);

      double completion = 0;
      MotivationalNotification motivationalNotification = dbServiceSubscriber
          .getMotivationalNotification(((SurveyActivity) mContext).getStudyId(), mRealm);
      if (total > 0) {
      ...
      if (motivationalNotification == null) {
        if (completion >= 100) {
          hundredPc = true;
          SetDialogHelper.setNeutralDialog(mContext,
              mContext.getResources().getString(R.string.study) + " " + title + " " + mContext
                  .getResources().getString(R.string.percent_complete1), false,
          ...
          SetDialogHelper.setNeutralDialog(mContext,
              mContext.getResources().getString(R.string.missed_activity) + " "
                  + ((SurveyActivity) mContext).getTitle1() + " " + mContext.getResources()
                  .getString(R.string.we_encourage), false,
              mContext.getResources().getString(R.string.ok),
              mContext.getResources().getString(R.string.app_name));
  ...
  public void updateStudyState(String completion, String adherence) {
    ...
    RegistrationServerConfigEvent registrationServerConfigEvent = new RegistrationServerConfigEvent(
        "post_object", URLs.UPDATE_STUDY_PREFERENCE, UPDATE_STUDY_PREFERENCE, mContext,
        LoginData.class, null, header, jsonObject, false, this);
  ...
  private Filter getFilterList() {
    ...
    if (filterPos == 0) {
      ...
      for (int i = 0; i < activitiesArrayList1.size(); i++) {
        if (activitiesArrayList1.get(i).getType() == null || activitiesArrayList1.get(i).getType()
  ...
  boolean isWithinRange(Date starttime, Date endtime) {
    if (endtime == null) {
      ...
      return (new Date().after(starttime) || new Date().equals(starttime)) && new Date()
          .before(endtime);
    }
  }
  ...
  public void getActivityInfo(String activityId, int currentRunId, String status, boolean branching,
      String activityVersion, ActivityStatus activityStatus, ActivitiesWS activitiesWS) {
    ...
    if (status.equalsIgnoreCase(YET_To_START)) {
      ...
      updateUserPreference(((SurveyActivity) mContext).getStudyId(), status, activityId,
          mCurrentRunId);
    } else {
  ...
  private void updateActivityInfo(String activityId) {
    ...
    HashMap<String, String> header = new HashMap();
    String url =
        URLs.ACTIVITY + "?studyId=" + ((SurveyActivity) mContext).getStudyId() + "&activityId="
            + activityId + "&activityVersion=" + mActivityVersion;
    WCPConfigEvent wcpConfigEvent = new WCPConfigEvent("get", url, ACTIVTTYINFO_RESPONSECODE,
        mContext, ActivityInfoData.class, null, header, null, false, this);
  ...
  private void launchSurvey(ActivityObj activity) {
    try {
      ...
      mActivityObj = dbServiceSubscriber
          .getActivityBySurveyId(((SurveyActivity) mContext).getStudyId(), mActivityId, mRealm);
      ...
      if (mActivityObj != null) {
        ...
        mTask = ActivityBuilder.create(mContext,
            ((SurveyActivity) mContext).getStudyId() + "_STUDYID_" + mActivityObj.getSurveyId()
                + "_" + mCurrentRunId, stepsBuilder.getsteps(), mActivityObj, mBranching,
            dbServiceSubscriber);
          ...
          if (locationPermission) {
            if ((ActivityCompat
              ...
              if (!hasPermissions(permission)) {
                ActivityCompat
                    .requestPermissions((Activity) mContext, permission, PERMISSION_REQUEST_CODE);
              } else {
                startsurvey();
              }
            } else {
  ...
  private void startsurvey() {
    ...
    Intent intent = CustomSurveyViewTaskActivity.newIntent(mContext,
        ((SurveyActivity) mContext).getStudyId() + "_STUDYID_" + mActivityObj.getSurveyId() + "_"
            + mCurrentRunId, ((SurveyActivity) mContext).getStudyId(), mCurrentRunId,
        mActivityStatus, mActivityStatusData.getMissedRun(), mActivityStatusData.getCompletedRun(),
        mActivityStatusData.getTotalRun(), mActivityVersion,
        mActivityStatusData.getCurrentRunStartDate(), mActivityStatusData.getCurrentRunEndDate(),
        mActivityObj.getSurveyId(), mBranching);
  ...
  public void updateUserPreference(String studyId, String status, String activityId,
      int activityRunId) {
    UpdatePreferenceEvent updatePreferenceEvent = new UpdatePreferenceEvent();
    AppController.getHelperProgressDialog().showProgress(mContext, "", "", false);
    HashMap<String, String> header = new HashMap();
    ...
    try {
      ...
      for (int i = 0; i < activityListData.getActivities().size(); i++) {
        if (activityListData.getActivities().get(i).getActivityId().equalsIgnoreCase(activityId)) {
          activityStatus
              .put("activityStartDate", activityListData.getActivities().get(i).getStartTime());
          activityStatus
              .put("activityEndDate", activityListData.getActivities().get(i).getEndTime());
          activityStatus.put("anchorDateVersion",
              activityListData.getActivities().get(i).getActivityVersion());
//                    activityStatus.put("anchorDatecreatedDate", );
          activityStatus.put("lastModifiedDate",
    ...
    try {
      ...
      OfflineData offlineData = dbServiceSubscriber
          .getActivityIdOfflineData(studyIdActivityId, mRealm);
      if (offlineData != null) {
      ...
      mDeleteIndexNumberDB = number;
      AppController
          .pendingService(mContext, number, "post_object", URLs.UPDATE_ACTIVITY_PREFERENCE, "",
              jsonObject.toString(), "registration", "", "", studyIdActivityId);
    } catch (Exception e) {
      e.printStackTrace();
    }
    //////////
    ...
    RegistrationServerConfigEvent registrationServerConfigEvent = new RegistrationServerConfigEvent(
        "post_object", URLs.UPDATE_ACTIVITY_PREFERENCE, UPDATE_USERPREFERENCE_RESPONSECODE,
        mContext, LoginData.class, null, header, jsonObject, false, this);
  ...
  private class ResponseData extends AsyncTask<String, Void, String> {
    ...
    protected String doInBackground(String... params) {
      ...
      if (connectionDetector.isConnectingToInternet()) {
        mResponseModel = HttpRequest
            .getRequest(URLs.PROCESSRESPONSEDATA + query, new HashMap<String, String>(),
                "Response");
    ...
    protected void onPostExecute(String response) {
      ...
      if (response != null) {
        if (response.equalsIgnoreCase("session expired")) {
          ...
          try {
            JSONObject jsonObject = new JSONObject(
                String.valueOf(mResponseModel.getResponseData()));
          ...
          try {
            if (anchorDateSchedulingDetails.getSourceType().equalsIgnoreCase("ActivityResponse")) {
              ...
              JSONObject jsonObject1 = (JSONObject) new JSONObject(String.valueOf(jsonArray.get(0)))
                  .get("data");
              ...
              stepRecordCustom.setActivityID(anchorDateSchedulingDetails.getStudyId() + "_STUDYID_"
                  + anchorDateSchedulingDetails.getSourceActivityId());
              stepRecordCustom.setStepId(anchorDateSchedulingDetails.getSourceKey());
              stepRecordCustom.setTaskStepID(anchorDateSchedulingDetails.getStudyId() + "_STUDYID_"
                  + anchorDateSchedulingDetails.getSourceActivityId() + "_" + 1 + "_"
            ...
            } else if (anchorDateSchedulingDetails.getSourceType()
                .equalsIgnoreCase("ParticipantProperty")) {
  ...
  private void callLabkeyService(int position) {
    if (mArrayList.size() > position) {
      ...
      if (anchorDateSchedulingDetails.getSourceType().equalsIgnoreCase("ActivityResponse")
        ...
        StepRecordCustom stepRecordCustom = dbServiceSubscriber.getSurveyResponseFromDB(
            anchorDateSchedulingDetails.getStudyId() + "_STUDYID_" + anchorDateSchedulingDetails
                .getSourceActivityId(), anchorDateSchedulingDetails.getSourceKey(), realm);
        if (stepRecordCustom != null) {
          ...
          String query =
              "sql=SELECT%20%22" + anchorDateSchedulingDetails.getSourceKey() + "%22%20FROM%20%22"
                  + anchorDateSchedulingDetails.getSourceActivityId() + anchorDateSchedulingDetails
                  .getSourceFormKey() + "%22&participantId=" + anchorDateSchedulingDetails
                  .getParticipantId();
        ...
        String query =
            "sql=SELECT%20%22" + anchorDateSchedulingDetails.getSourceKey() + "%22%20FROM%20%22"
                + "ParticipantProperties" + "%22&participantId=" + anchorDateSchedulingDetails
                .getParticipantId();
        new ResponseData(position, anchorDateSchedulingDetails, query).execute();
      } else {
        callLabkeyService(position + 1);
